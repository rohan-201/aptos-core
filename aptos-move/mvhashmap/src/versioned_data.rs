...
impl<K: Hash + Clone + Debug + Eq, V: TransactionWrite> VersionedData<K, V> {
    ...
    // If the result is err, it means the base value hadn't been set.
    pub(crate) fn commit_delta(&self, key: &K, txn_idx: TxnIndex) -> anyhow::Result<u128> {
        let mut v = self.values.get_mut(key).expect("Path must exist");
        ...
        match v.read(txn_idx + 1) {
            ...
            Err(MVDataError::Unresolved(_)) => bail!("No base value"),
