...
impl TransactionGenerator {
    ...
    pub fn run_workload(
        ...
        assert!(self.block_sender.is_some());
        let mut transaction_generator = transaction_generator_creator.create_transaction_generator();
            ...
            let transactions: Vec<_> = (0..(block_size / transactions_per_sender))
                ...
                .flat_map(|_| {
                    let sender = self
                        ...
                        .unwrap()
                        .get_random();
                    transaction_generator.generate_transactions(vec![sender], transactions_per_sender)
                        .into_iter()
                        .map(|t| BenchmarkTransaction{transaction: Transaction::UserTransaction(t), extra_info: None})
                        .collect::<Vec<_>>()
                })
                .chain(once(
