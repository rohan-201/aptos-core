...
use aptos_transaction_generator_lib::{create_txn_generator_creator, TransactionType, TransactionGeneratorCreator};
use gen_executor::DbGenInitTransactionExecutor;
use tokio::runtime::Runtime;
use transaction_generator::MAX_ACCOUNTS_INVOLVED_IN_P2P;
use std::{fs, path::Path, time::Instant, sync::{atomic::AtomicUsize, Arc}};
    ...
    V: TransactionBlockExecutor<BenchmarkTransaction> + 'static,
{
    ...
    let transaction_generator_creator = transaction_type.map(|transaction_type| init_workload::<V, _>(transaction_type, db.clone(), &source_dir));
    ...
    if let Some(transaction_generator_creator) = transaction_generator_creator {
        generator.run_workload(block_size, num_transfer_blocks, transaction_generator_creator, transactions_per_sender);
    } else  {
...
{
    let version = db.reader.get_latest_version().unwrap();
    let (pipeline, block_sender) = Pipeline::<V>::new(BlockExecutor::new(db.clone()), version, false);
    ...
    let (txn_generator_creator, _address_pool, _account_pool) = runtime.block_on(async {
        ...
        create_txn_generator_creator(
            ...
            1,
            &mut accounts_cache.accounts_mut(),
